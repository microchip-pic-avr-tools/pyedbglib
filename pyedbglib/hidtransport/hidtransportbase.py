...
from .toolinfo import tool_shortname_to_product_string_name, get_default_report_size
from .toolinfo import detect_hid_packet_size, adjust_hid_packet_size
...
class HidTransportBase(object):
    ...
    def get_matching_tools(self, serial_number_substring='', product=None):
        ...
        toolname_in_product_string = tool_shortname_to_product_string_name(product)
    ...
    def connect(self, serial_number=None, product=None):
        ...
        # At this point we have identified a single usb device to which we want to connect
        self.device = matching_devices[0]

        # HID endpoint size is static for most tools, so look up the default
        packet_size = get_default_report_size(self.device.product_id)

        # Some tools have configurable packet size for HID endpoints.
        # Depending on the OS, the configuration can be detected pre-connect, or adjusted post-connect

        # Pre-connect EP size detection
        detected_packet_size = detect_hid_packet_size(self.device.product_id, self.device.serial_number)
        if detected_packet_size and detected_packet_size != packet_size:
            self.logger.debug("Overriding default HID packet size with detected size")
            packet_size = detected_packet_size
        ...
        # Post-connect EP size adjustment
        adjust_hid_packet_size(self, self.device)
        self.hid_info()
        return True
