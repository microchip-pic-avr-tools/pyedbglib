...
class Jtagice3Command(AvrCommand):
    ...
    # Handlers which publish events
    EVENT_HANDLER_IDS = [HANDLER_HOUSEKEEPING, HANDLER_AVR8_GENERIC]
    ...
    def validate_response(self, response):
        ...
        :raises PyedbglibError: if validation fails
        """
        self.logger.debug("Checking response (%s)", print_helpers.bytelist_to_hex_string(response))

        # Check length first
        if len(response) <= self.JTAGICE3_HEADER_RESPONSE_PAYLOAD_START:
            raise PyedbglibError("Invalid response length ({:d}).".format(len(response)))

        # Check token
        if response[self.JTAGICE3_HEADER_RESPONSE_SOF_TOKEN] != self.JTAGICE3_TOKEN:
            raise PyedbglibError("Invalid token (0x{:02X}) in response."
                                 .format(response[self.JTAGICE3_HEADER_RESPONSE_SOF_TOKEN]))

        # Check sequence
        sequence = binary.unpack_be16(response[self.JTAGICE3_HEADER_RESPONSE_SEQUENCE:
                                               self.JTAGICE3_HEADER_RESPONSE_SEQUENCE+2])
        ...
        # Check handler
        if response[self.JTAGICE3_HEADER_RESPONSE_HANDLER_ID] != self.handler:
            raise PyedbglibError("Invalid handler (0x{:02X}) in response."
                                 .format(response[self.JTAGICE3_HEADER_RESPONSE_HANDLER_ID]))

    def jtagice3_command_response_raw(self, command):
        """
        Sends a JTAGICE3 command and receives the corresponding response

        :param command: bytearray command to send
        """
        # Header
        header = bytearray([self.JTAGICE3_TOKEN,
                            self.JTAGICE3_PROTOCOL_VERSION,
                            self.sequence_id & 0xFF, (self.sequence_id >> 8) & 0xFF,
                            self.handler])
    ...
    def jtagice3_command_response(self, command):
        ...
        # Peel and return
        return response[self.JTAGICE3_HEADER_RESPONSE_PAYLOAD_START:]
...
class Jtagice3Protocol(Jtagice3Command):
    ...
    # Protocol frames - base responses
    ...
    def peel_response(self, response, expected=None):
        ...
        # Special handling
        if expected is not None and response[self.PROTOCOL_HEADER_RESPONSE_ID] == expected:
            return_list = True, response[self.PROTOCOL_HEADER_RESPONSE_PAYLOAD:]
        else:
            if response[self.PROTOCOL_HEADER_RESPONSE_ID] == self.PROTOCOL_OK:
                return_list = True, []
            elif response[self.PROTOCOL_HEADER_RESPONSE_ID] == self.PROTOCOL_LIST:
                return_list = True, response[self.PROTOCOL_HEADER_RESPONSE_PAYLOAD:]
            elif response[self.PROTOCOL_HEADER_RESPONSE_ID] == self.PROTOCOL_DATA:
                # Trailing status is not included on some handlers
                if self.supports_trailing_status and response[-1] == self.FAILURE_OK:
                    return_list = True, response[self.PROTOCOL_HEADER_RESPONSE_PAYLOAD:-1]
                else:
                    return_list = False, [response[-1]]
            elif response[self.PROTOCOL_HEADER_RESPONSE_ID] == self.PROTOCOL_FAILED:
                return_list = False, [response[self.PROTOCOL_HEADER_RESPONSE_PAYLOAD]]
    ...
    def query(self, context):
        ...
        :raises PyedbglibError: if the query fails
        """
    ...
    def set_byte(self, context, offset, value):
        ...
        """
        if not 0 <= value <= 0xFF:
            raise PyedbglibError("Value out of 8-bit range!")
    ...
    def set_le16(self, context, offset, value):
        ...
        if not 0 <= value <= 0xFFFF:
            raise PyedbglibError("Value out of 16-bit range!")
    ...
    def set_le32(self, context, offset, value):
        ...
        if not 0 <= value <= 0xFFFFFFFF:
            raise PyedbglibError("Value out of 32-bit range!")
    ...
    def _set_protocol(self, context, offset, data):
        ...
        :raises PyedbglibError: if the set fails
        """
    ...
    def poll_events(self):
        ...
        event = super().poll_events()
        if not event:
            return None
        self.logger.debug("Decoding incoming event")

        # Unpack content in JTAGICE3 format
        if event[Jtagice3Command.JTAGICE3_HEADER_EVENT_SOF_TOKEN] == Jtagice3Command.JTAGICE3_TOKEN:
            self.logger.debug("Decoding JTAGICE3 event")
            # Ignore version, sequence; check that its a valid JTAGICE3 type
            if event[Jtagice3Command.JTAGICE3_HEADER_EVENT_HANDLER_ID] in self.EVENT_HANDLER_IDS:
                # Include HANDLER ID in event passed back
                return event[Jtagice3Command.JTAGICE3_HEADER_EVENT_PAYLOAD_START-1:]
            self.logger.debug("Unhandled incoming event ignored.")
        return None
