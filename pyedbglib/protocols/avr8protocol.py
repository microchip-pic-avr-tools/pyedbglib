...
class Avr8Protocol(Jtagice3Protocol):
    ...
    CMD_AVR8_EXECUTE_PATCH = 0x70  # Execute a list of instructions
    ...
    AVR8_FAILURE_VTG_TOO_LOW_FOR_FEATURE = 0x54  # Target voltage is too low to use this feature
    ...
    EVT_AVR8_BREAK_PC = 1 # Program counter, 4 bytes, LE
    EVT_AVR8_BREAK_CAUSE = 5 # Break cause
    EVT_AVR8_BREAK_EXT_INFO = 6 # Extended info, 2 bytes
    ...
    AVR8_CTXT_TEST = 0x80  # Alternative means for testing target run state

    # Test context - retrieve target run state.  Alternative to event polling for development.
    AVR8_TEST_TGT_RUNNING = 0x00  # Responds with running=1; stopped=0
    ...
    # Useful values in the OCD memtype area.  Note: UPDI only!
    AVR8_MEMTYPE_OCD_PC = 0x14  # Program counter
    AVR8_MEMTYPE_OCD_SP = 0x18  # Stack pointer
    AVR8_MEMTYPE_OCD_SREG = 0x1C  # Status register
    ...
    def debugwire_disable(self):
        """Disables debugWIRE interface temporarily"""
        self.logger.debug("debugWIRE disable")
        self.check_response(self.jtagice3_command_response(bytearray([self.CMD_AVR8_DISABLE_DEBUGWIRE,
                                                                      self.CMD_VERSION0])))
    ...
    def program_counter_write(self, program_counter):
        ...
        :param program_counter: new program counter value
        """
        self.check_response(self.jtagice3_command_response(
    ...
    def decode_break_event(self, event):
        """
        ...
        :param event: event information for parsing
        :returns: Program Counter value if it is a break event, None otherwise
        """
        self.logger.debug("Checking possible BREAK event")
        # Sanity check for length before parsing
        if not event or len(event) < self.EVT_AVR8_BREAK_EXT_INFO+2+1:
            return None
        # Parse out protocol fields
        if event[0] == Jtagice3Protocol.HANDLER_AVR8_GENERIC:
            self.logger.debug("Decoding AVR8 GENERIC event")
            # Advance to EVT_AVR8_BREAK_EVT_ID
            event = event[1:]
            if event[Avr8Protocol.EVT_AVR8_BREAK_EVT_ID] == Avr8Protocol.EVT_AVR8_BREAK:
                self.logger.debug("Decoding AVR8 BREAK event")
                # TODO - add parsing for OCD variants which use additional fields
                program_counter = binary.unpack_be32(event[Avr8Protocol.EVT_AVR8_BREAK_PC:
                                                           Avr8Protocol.EVT_AVR8_BREAK_PC+4])
                return program_counter
        return None
