...
class MpLabTransport(object):
    ...
    * get_report_size for determining the HID report size in use
    ...
    def hid_transfer(self, packet):
        ...
        # Send
        numbytes = self.mplabcomm.Send(packet, len(packet))
        if numbytes < 0:
            self.logger.error("Error while sending using MPLABCOMM")
            raise IOError("Unrecoverable error while communicating with the PKOB nano.  Please reconnect the kit and restart the session.")
